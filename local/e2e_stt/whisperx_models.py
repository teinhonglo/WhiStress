...
class SpeechModel(object):
    def __init__(self, tag="large-v2", device="cuda", language="en", condition_on_previous_text=False, args=None):
        # Fluency
        ...
        self.decode_options = {"suppress_tokens": suppress_tokens, "repetition_penalty": 1.5, "no_repeat_ngram_size": 2 }
    ...
    def set_initial_prompt(self, initial_prompt=None):
        self.decode_options["initial_prompt"] = initial_prompt
        self.model = whisperx.load_model(self.tag, self.device, compute_type=self.compute_type, language=self.language, asr_options=self.decode_options)
        print(f"initial_prompt {initial_prompt}")

    # STT features
    def recog(self, wav_path, text_prompt=None):
        speech, rate = sf.read(wav_path)
        result = self.model.transcribe(wav_path, batch_size=self.batch_size)
        # merged results
        ...
        try:
            ...
            start_time = 0
            end_time = int(len(speech) / self.sample_rate)
            
        results = {"segments": [{"text": text_norm, "start": start_time, "end": end_time}], "language": self.language }
        
        result = whisperx.align(results["segments"], self.model_a, self.metadata, wav_path, self.device, return_char_alignments=False)
        '''
    ...
    def sil_feats(self, ctm_info, total_duration):
        ...
        '''
        {sil, long_sil}_rate1: silence["summ"] / response_duration
        {sil, long_sil}_rate2: num_silences / num_words
        '''
        sil_dur = sil_stats["sil_summ"]
        long_sil_dur = long_sil_stats["long_sil_summ"]
        num_sils = len(sil_list)
        num_long_sils = len(long_sil_list)
        num_words = len(ctm_info)
        
        sil_stats["sil_rate1"] = sil_dur / response_duration
        ...
        long_sil_stats["long_sil_rate1"] = long_sil_dur / response_duration 
    ...
    def word_feats(self, ctm_info, total_duration):
        ...
        word_freq2 = word_duration_stats["word_duration_summ"] / response_duration
    ...
    def phone_feats(self, ctm_info, total_duration):
        ...
        for phone, start_time, duration, conf in ctm_info:
            ...
            phone = phone.rstrip(string.digits)
        ...
        phone_freq2 = phone_duration_stats["phone_duration_summ"] / response_duration
        
        phone_basic_dict = { 
                            "phone_count": phone_count,
                            "phone_freq": phone_freq,
                            "phone_freq2": phone_freq2
                           }
